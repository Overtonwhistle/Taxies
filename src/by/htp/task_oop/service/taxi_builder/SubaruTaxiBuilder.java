...
public class SubaruTaxiBuilder extends TaxiCarBuilder {
	...
	private final String BRAND_NAME = "SUBARU";
	private final int DEFAULT_YEAR = 2011;
	private final int DEFAULT_SEATS = 4;
	private final int DEFAULT_AIRBAGS = 4;
	private final double DEFAULT_ENGINE_DISPLACEMENT = 1500;
	private final double DEFAULT_MAX_SPEED = 155;
	private final double DEFAULT_FUEL_CONSUMPTION = 8.1;
	private final double DEFAULT_PRICE = 14000;
	...
	public void buildModel(String model) throws TaxiBuildingException {
		...
		try {
			subaruModel = SubaruModel.valueOf(model.toUpperCase().trim());

		} catch (IllegalArgumentException e) {
			throw new TaxiBuildingException("Incorrect model!");
		}
		taxiCar.setModel(subaruModel.toString());
	}
